#!/usr/bin/env python3
"""
S4_C: Epithelial density spatial maps for all 10 GSE251950 samples.
2×5 grid showing Total_Epi proportion per spot.
"""
...
def main():
    print("S4_C: Epithelial Density Spatial Maps (All 10 Samples)")
    ...
    vmin = df['Total_Epi'].quantile(0.02)
    vmax = df['Total_Epi'].quantile(0.98)
    ...
    for idx, (sample_name, label) in enumerate(SAMPLE_INFO):
        ...
        sc = ax.scatter(sample_df['x'], sample_df['y'],
                        c=sample_df['Total_Epi'],
                        cmap='Greens', s=6, alpha=0.8,
    ...
    cbar.set_label('Total epithelial\nfraction', fontsize=5 * SCALE)
