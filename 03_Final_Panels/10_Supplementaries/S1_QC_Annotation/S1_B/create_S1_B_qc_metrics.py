...
# Standard 4-group colors
COLOR_PRE_R  = '#bde0fe'
COLOR_PRE_NR = '#a2d2ff'
COLOR_POST_R = '#ffcfd2'
COLOR_POST_NR = '#f1c0e8'
COLOR_UNSEL  = '#d9d9d9'  # gray for not selected

OUTPUT_DIR = Path(__file__).parent
OUTPUT_FILE = OUTPUT_DIR / "panel_S1_B.png"

# =============================================================================
# Load data — stomach only
# =============================================================================
print("Loading data...")
...
def assign_group(row):
    ...
    post = str(row['stomach_post_grouping'])
    if pre == 'Responsed':
        return 'Pre-R'
    elif pre == 'No-response':
        return 'Pre-NR'
    elif post == 'Responsed':
        return 'Post-R'
    elif post == 'No-response':
        return 'Post-NR'
    else:
        return 'Not selected'

sample_info['group'] = sample_info.apply(assign_group, axis=1)

# Sort: Pre-R, Pre-NR, Post-R, Post-NR, Not selected
group_order = {'Pre-R': 0, 'Pre-NR': 1, 'Post-R': 2, 'Post-NR': 3, 'Not selected': 4}
...
color_map = {
    ...
    'Post-R': COLOR_POST_R, 'Post-NR': COLOR_POST_NR,
    'Not selected': COLOR_UNSEL,
}
sample_to_group = dict(zip(sample_info['sample'], sample_info['group']))

for g in group_order:
    n = sum(sample_info['group'] == g)
    if n > 0:
        print(f"  {g}: {n} samples")

# Find group boundaries for vertical separators
boundaries = []
prev_group = None
...
for ax, metric, title, ylabel in zip(axes, metrics, titles, ylabels):
    colors = [color_map[sample_to_group[s]] for s in sample_order]
...
# Legend
legend_handles = [
    mpatches.Patch(facecolor=COLOR_PRE_R, edgecolor='black', alpha=0.8, label='Pre-R'),
    mpatches.Patch(facecolor=COLOR_PRE_NR, edgecolor='black', alpha=0.8, label='Pre-NR'),
    mpatches.Patch(facecolor=COLOR_POST_R, edgecolor='black', alpha=0.8, label='Post-R'),
    mpatches.Patch(facecolor=COLOR_POST_NR, edgecolor='black', alpha=0.8, label='Post-NR'),
    mpatches.Patch(facecolor=COLOR_UNSEL, edgecolor='black', alpha=0.8, label='Not selected'),
]
