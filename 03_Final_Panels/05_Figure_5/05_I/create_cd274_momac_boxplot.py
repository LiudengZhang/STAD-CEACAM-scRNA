...
#: Two lines of body type and their leading, given up by the axes.
...
def main():
    ...
    finish_two_group(
        fig, ax, title='Monocytes/\nMacrophages',
        tick_labels=[f'R\n(n={len(r_data)})', f'NR\n(n={len(nr_data)})'],
        positions=[0, 1], width=0.5, ylabel_markup='PD-L1 (*CD274*) expression',
        letter_cell=LETTER_CELL, panel_w_mm=PANEL_W_MM, panel_h_mm=PANEL_H_MM,
        top_extra_mm=0.27, bottom_mm=6.30)
