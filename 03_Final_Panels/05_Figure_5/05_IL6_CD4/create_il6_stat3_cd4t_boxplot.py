...
def main():
    ...
    # its leading in width, and this panel is 18.5 mm wide.
    ax.set_ylabel('IL-6/JAK/STAT3 Score')
    # The gene set is named in full in the axis label, so the title carries
    # the cell type alone, as the box titles of panel J do. A title wider than
    # the plotting box cannot be fitted at all: it is centred on that box, so
    # every millimetre the box is narrowed only moves the title's edge by half
    # a millimetre.
    ax.set_title('CD4+ T')
