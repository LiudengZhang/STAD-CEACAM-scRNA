#!/usr/bin/env python3
"""
Panel J: Spatial MoMac Deconvolution Fraction (single panel, sample_03)
Extracted from old 03_H left panel. 4x scaling — ONLY canvas + fonts scaled.
"""
...
def main():
    ...
    sc = ax.scatter(sample_df['x'], sample_df['y'],
                    ...
                    cmap='YlOrRd', s=6, alpha=0.8)
    cbar = plt.colorbar(sc, ax=ax, fraction=0.046, pad=0.04)
    cbar.solids.set_rasterized(False)
    cbar.set_label('MoMac Score', fontsize=7 * SCALE)
    cbar.ax.tick_params(labelsize=6 * SCALE, width=1.0, length=4)
    cbar.outline.set_linewidth(1.0)

    ax.set_title('MoMac Fraction', fontsize=8 * SCALE, fontweight='normal')
    ...
    out = os.path.join(BASE_DIR, 'spatial_momac_fraction.png')
