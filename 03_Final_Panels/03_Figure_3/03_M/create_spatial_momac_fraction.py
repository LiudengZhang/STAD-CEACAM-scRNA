#!/usr/bin/env python3
"""
Figure 3 panel M - the monocyte/macrophage fraction across the spatial spots of one Visium
section.
...
rather than typed, and the panel-letter corner is left clear for the assembler.
  ...
  printed panel  Figure 3 M       (PROVENANCE.csv; the directory name agrees,
...
Monocytes/Macrophages column, the same colour map, the same alpha, the same
...
def main():
    ...
    print("=" * 60)
    print("Figure 3 panel M: spatial MoMac fraction")
    print("=" * 60)
    ...
    print(f"  MoMac range: {sample_df['Monocytes/Macrophages'].min():.3f} \u2013 "
          f"{sample_df['Monocytes/Macrophages'].max():.3f}")
    ...
    sc = ax.scatter(sample_df['x'], sample_df['y'],
                    ...
                    cmap='YlOrRd', s=2.5 * AREA, alpha=0.8)
    ...
    ax.set_title('MoMac Fraction', fontweight='normal')
