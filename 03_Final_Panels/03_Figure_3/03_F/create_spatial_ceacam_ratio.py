#!/usr/bin/env python3
"""
Figure 3 panel F - the CEACAM ratio across the spatial spots of one Visium
section.
...
rather than typed, and the panel-letter corner is left clear for the assembler.
  ...
  printed panel  Figure 3 F       (PROVENANCE.csv; the directory name agrees,
                                   but the letter was looked up, not inferred)

Every spot, every coordinate, every colour value, every colour map and every
string is the earlier drawing's: the same section, the same CEACAM_ratio
column, the same cmap, the same alpha, the same fraction/pad colour bar and the
same equal aspect.
...
def main():
    ...
    print("=" * 60)
    print("Figure 3 panel F: spatial CEACAM ratio")
    print("=" * 60)
    ...
    sc = ax.scatter(sample_df['x'], sample_df['y'],
                    ...
                    cmap='RdBu_r', s=2.5 * AREA, alpha=0.8)
    ...
    ax.set_title('CEACAM Ratio', fontweight='normal')
    ...
    # 'datalim': the frame is the axes box and the map is centred in it, so
    # the frame can be pinned to the row's line below.
    ...
    out = os.path.join(BASE_DIR, 'spatial_ceacam_ratio')
