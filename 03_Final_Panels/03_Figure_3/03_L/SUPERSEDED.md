# Figure 3 panel L — superseded, not broken

Ruling-date: 2026-09-10
Ruled-by: the author (CIR-26-0753-ET major revision)
Panel: Figure 3 **L** — the printed letter, looked up in
`03_Revised_Panels/PROVENANCE.csv`. The directory `03_L` happens to agree
here; four Figure 3 panels do not, so the letter was looked up and not
inferred.
Script: `create_spatial_dist_immune.py`
...
figure (`00_GROUND_TRUTH/figures/Figure 3.pdf`): a bar reading 500 / 1000 / 1500 / 2000
with no unit anywhere on the panel, under the title `Distance to Immune`. The redrawn
panel prints 200 / 400 / 600 under `Distance to Immune` with `(µm)` on a second line, and
...
The panel scattered the `distance_to_immune` column of `spot_data.csv` directly. That
...
drawn and the ink read back off the PDFs:
    ...
    published                    title x 3.66-27.12 mm, y 1.91-4.38
                                 bar at x 29.08 mm, ticks 500 / 1000 / 1500 / 2000
    "Distance to Immune (µm)"      one line   title x 0.60-29.17, y 4.12-6.59
                                 ticks 0 / 1000
    "µm" on the bar              title x 0.60-23.10, y 4.02-6.49
                                 ticks 0 / 1000
    "Distance to Immune\n(µm)"    ADOPTED    title x 3.66-27.12, y 0.52-2.99
                                 bar at x 29.08 mm, ticks 200 / 400 / 600
...
## What moved, exactly
    ...
    colour bar limit     2137.125 -> 614.106
    colour bar ticks     matplotlib's locator on the new limit
    title                'Distance to Immune' -> 'Distance to Immune' + a second line '(µm)'
    ...
    canvas               the same millimetres, drawn 1:1 into the same measured
                         printed rectangle 35.5,88.2,68.7,114.6 mm
...
- `PROVENANCE.csv`, Figure 3 panel L: `reproduces_published = no`, with this
