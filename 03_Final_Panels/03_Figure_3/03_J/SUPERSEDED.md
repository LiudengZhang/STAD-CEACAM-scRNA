# Figure 3 panel J — superseded, not broken

Ruling-date: 2026-09-10
Ruled-by: the author (CIR-26-0753-ET major revision)
Panel: Figure 3 **J** — the printed letter, looked up in
`03_Revised_Panels/PROVENANCE.csv`. The directory `03_J` happens to agree here;
four Figure 3 panels do not, so the letter was looked up and not inferred.
Script: `create_spatial_boxplot_immune.py`
...
tick values 0, 250, 500, 750, 1000. The redrawn panel prints
`Distance (µm)` and 0, 100, 200, 300, and does not reproduce the
printed axis. Nothing else moved.

## Why

The panel plotted the `distance_to_immune` column of `spot_data.csv`
...
difference 179 µm". Those were array units with a micrometre sign on them.
...
reads 51 µm after conversion, from 179 before.
...
recorded rather than taken quietly:
  ...
  * a per-section factor would re-weight the ten pairs against each other,
    ...
    51.79 um where the cohort factor gives 51.47 um for this panel's
...
- `PROVENANCE.csv`, Figure 3 panel J: `reproduces_published = no`, with this
