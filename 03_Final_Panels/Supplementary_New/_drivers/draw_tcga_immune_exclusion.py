...
builds three things and passes them to `_panel(d, t, purity_col)`; each is
reconstructed from what `main()` already put on disk or from the stored tables
it read, never from the raw counts:
  ...
  `d`   is the per-tumour frame. `main()` assembles it from
          - `est`, the ESTIMATE score matrix, which it writes to
            ...
            shells out to R and needs the tumour-only count matrix and an
            Ensembl-to-symbol map from a raw GDC file. Neither is opened.
          ...
          - `ab`, PanCanAtlas ABSOLUTE purity, likewise read as `main()`
            reads it.
        The barcode truncation, the intersection and the column order all
        follow `main()` line for line, so `d` has the same rows in the same
        order. Verified: no duplicate 12-character barcode in either table.

  `purity_col` is hardcoded by `main()` rather than derived from the columns
        present, so it is hardcoded here too, next to the column that carries
        it. `_panel` takes the argument and does not use it; it is passed for
        fidelity.

    python draw_tcga_immune_exclusion.py --check
    python draw_tcga_immune_exclusion.py
"""
...
# the x labels at 5.5 x that scale.
A_FIT = 0.2625
A_TYPE = 5.5 * 4 * A_FIT
...
def draw_B(fr, save=True):
    ...
    # prove the same regression line is drawn.
