...
# /path/to/machine/shared-storage, where every input to these pipelines lives. A pod
...
run_bayesprism_tcga() {
    ...
    (cd "$d" && conda run -n r_bayesprism Rscript step2_run_bayesprism_tcga.R) \
    ...
    (cd "$d" && conda run -n r_bayesprism Rscript step4_extract_epi_cd8_pinned.R) \
...
run_bayesprism_tcga_step1() {
    ...
    (cd "$d" && conda run -n stad_ceacam python step1_prepare_tcga_bulk_pinned.py) \
...
run_bayesprism_tiger() {
    ...
    if [ ! -s "$d/sc_counts.tsv" ]; then
        ...
        (cd "$d" && conda run -n stad_ceacam python step1_prepare_reference.py) \
    ...
    (cd "$d" && conda run -n r_bayesprism Rscript step2_run_bayesprism.R) \
        > "$LOGS/bayesprism_tiger.log" 2>&1
    note "BayesPrism Tiger: exit $?"
}

# Two steps, both named rather than globbed. `find .../scripts -name "*.R" |
# head -1` returned the archived upstream-pipeline prototype 01_Prototype/
...
run_nichenet() {
    ...
    conda run -n stad_ceacam python "$d/02_Scripts/01_prepare_data.py" \
    ...
    conda run -n r_bayesprism Rscript "$d/02_Scripts/02_run_nichenet_c3mac.R" \
