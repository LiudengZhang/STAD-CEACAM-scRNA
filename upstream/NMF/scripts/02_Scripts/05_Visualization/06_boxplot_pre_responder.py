...
"""06_boxplot_pre_responder.py - Pre-treatment R vs NR boxplot"""
...
pre_df = scores_df[scores_df['timepoint'] == 'Pre'].copy()
responders = pre_df[pre_df['response_status'] == 'Responder']
non_responders = pre_df[pre_df['response_status'] == 'NonResponder']
...
for idx, mp in enumerate(sorted(mp_cols)):
    ...
    plot_data = [{'Response': r['response_status'], 'Activity': r[mp]} for _, r in pre_df.iterrows()]
    plot_df = pd.DataFrame(plot_data)

    colors = {'Responder': '#4CAF50', 'NonResponder': '#F44336'}
...
plt.suptitle('Pre-Treatment: Responder vs Non-Responder', fontsize=14, fontweight='bold', y=1.02)
plt.tight_layout()
plt.savefig(OUTPUT_DIR / "06_boxplot_pre_RvsNR.png", dpi=300, bbox_inches='tight', facecolor='white')
plt.close()
