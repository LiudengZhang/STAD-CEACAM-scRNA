...
"""
Reproduce upstream-pipeline t-test GSEA radar data exactly.
...
# upstream-pipeline data dir for Mast/Plasma (not in submission-tree)
R4_DATA = Path("/path/to/upstream-pipeline"
               "/01_standardized-pipeline/01_For_Annotation/02_Results/level_1")
...
CELL_TYPE_CONFIG = {
    ...
    "Mast_cells":        {"path": R4_DATA / "Mast_cells_v2/mast_cell_integrated.h5ad", "label": "Mast"},
    ...
    "Plasma_cells":      {"path": R4_DATA / "Plasma_cells/plasma_cell_integrated.h5ad", "label": "Plasma"},
    "TCD4_cells":        {"path": TCD4_H5AD,           "label": "CD4+ T"},
    "TCD8_cells":        {"path": TCD8_H5AD,           "label": "CD8+ T"},
}

# NR vs R (same as upstream-pipeline)
COMPARISONS = {
...
def main():
    print("=" * 60)
    print("Reproducing upstream-pipeline t-test GSEA (NR vs R, 100 perms)")
    print("=" * 60)
    ...
    # Compare with backup
    backup_csv = Path("/path/to/upstream-pipeline"
                      "/04_Final_Panels/05_Figure_5/05_F/nfkb_rankings_13types_mast_nr_vs_r_flipped.csv")
    if backup_csv.exists():
        old = pd.read_csv(backup_csv)
        print("\n\n=== COMPARISON WITH ROUND_4 ORIGINAL ===")
