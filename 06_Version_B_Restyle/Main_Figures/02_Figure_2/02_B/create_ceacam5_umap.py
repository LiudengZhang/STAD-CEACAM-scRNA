#!/usr/bin/env python3
"""
Figure 2, printed panel B, RESTYLED (Version B) - CEACAM5 expression UMAP on
epithelial cells.
...
The drawing code is the same code.
  ...
  printed panel  Figure 2 B     (PROVENANCE.csv; the directory letter happens
                                 to agree here - it was still looked up. Note
                                 Version A's own docstring calls it "Panel 2C",
                                 which is the pre-submission lettering and is
                                 wrong; PROVENANCE.csv is the authority.)
  printed rect   25.3 x 22.6 mm    (panel_rects.csv)
...
MARK
    ...
    The panel grew from 25.3 x 22.6 mm to 62 x 52 mm. The title and the
...
PRINTED_MM = (25.3, 22.6)           # published rect, panel_rects.csv
...
def main():
    ...
    style.save_panel(fig, Path(OUTPUT_DIR) / "ceacam5_expression_umap")
    print(f"Saved: {Path(OUTPUT_DIR) / 'ceacam5_expression_umap'}"
