#!/usr/bin/env python3
"""
Figure 5 panel K, RESTYLED (Version B) - NF-kB score against the Tex
...
The 200-gene Hallmark NF-kB list, the 19-gene Tex list, the >= 20-cell sample
filter, `sc.tl.score_genes`, the Spearman test and the P-value formatting are
Version A's, unchanged. The drawing code is the same code.

  printed panel  Figure 5 K        (PROVENANCE.csv; NOT inferred from "05_O")
  printed rect   22.5 x 28.7 mm    (panel_rects.csv)
  Version B box  50.0 x 55.0 mm

    The axes are square (`set_box_aspect(1)`), and at 7/8 pt the two-line title,
    the two-line y axis label and the x axis label need about 15 mm of the width
    and 19 mm of the height before any point is drawn. 22.5 mm cannot carry
    that; the growth is the point of the rebuild. Nothing plotted moves.
...
MARK
    ...
    The marker area, the marker edge width and the dashed trend line are scaled
    by those. Tick widths/lengths and spine widths are not: they are style, and
    cnsplots sets them.

Judgement call: the title's explicit `fontweight='normal'` is dropped so
cnsplots' bold axis title applies (PANEL_SPEC: follow the library). Its
`linespacing=1.4` is kept - that is layout of the two-line string, not type.
"""
...
PRINTED_MM = (22.5, 28.7)
PANEL_W_MM, PANEL_H_MM = 50.0, 55.0
MARGIN = dict(left=14.5, right=2.0, top=9.5, bottom=10.0)
...
STATE_GENES = [
    'PDCD1','CTLA4','HAVCR2','LAG3','TIGIT','CD244','CD160','TOX','TOX2','EOMES',
    'PRDM1','NR4A1','NR4A2','NR4A3','ENTPD1','CD38','CXCL13','LAYN','TNFRSF9',
]
