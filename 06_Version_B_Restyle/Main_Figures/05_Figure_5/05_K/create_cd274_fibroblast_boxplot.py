...
# Original one-tailed version: Round_5/03_Final_Panels/05_Figure_5/05_K/create_cd274_fibroblast_boxplot.py
"""
Figure 5 panel J (one of four), RESTYLED (Version B) - CD274 Post-R vs Post-NR
in Fibroblasts.

Version A is
`03_Revised_Panels/Main_Figures/05_Figure_5/05_K/create_cd274_fibroblast_boxplot.py`
...
string is Version A's. `np.random.seed(0)` stays exactly where Version A put it.

  printed panel  Figure 5 J        (PROVENANCE.csv; NOT inferred from "05_K")
  ...
  printed rect   86.8 x 28.7 mm    (panel_rects.csv, all four together;
                 ~21.7 x 28.7 mm each)
  Version B box  38.0 x 48.0 mm    (narrower than 05_I: this panel's y axis
                 label is the empty string, so it needs no label column)
...
PANEL_W_MM, PANEL_H_MM = 38.0, 48.0
MARGIN = dict(left=9.0, right=2.0, top=10.0, bottom=9.0)
