#!/usr/bin/env python3
"""
Figure 3 panel F, RESTYLED (Version B) - the CEACAM ratio across the spatial spots
of one Visium section.

NOTE THE PANEL LETTER. This directory is `03_F` and PROVENANCE.csv
confirms it holds printed panel **F** - but the letter was still looked up
...
same `CEACAM_ratio` column, the same `cmap='RdBu_r'`, the same `alpha`, the same
`fraction`/`pad` colour bar and the same equal aspect.

  printed panel  Figure 3 F     (PROVENANCE.csv; NOT inferred from "03_F")
  printed rect   30.1 x 27.8 mm   (panel_rects.csv)
...
PRINTED_MM = (30.1, 27.8)          # published rect, panel_rects.csv
...
def main():
    ...
    sc = ax.scatter(sample_df['x'], sample_df['y'],
                    ...
                    cmap='RdBu_r', s=6 * AREA, alpha=0.8)
