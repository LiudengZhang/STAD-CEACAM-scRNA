...
# Original one-tailed version: Round_5/03_Final_Panels/03_Figure_3/03_I/create_spatial_boxplot_stroma.py
"""
Figure 3 panel I, RESTYLED (Version B) - distance to stroma, CEACAM-high vs
CEACAM-low, paired within section.

NOTE THE PANEL LETTER, AND THE FILENAME. This directory is `03_I` and
PROVENANCE.csv says it holds printed panel **I** (CLAUDE.md rule 2: the
...
`y_max_limit=2300` and the bracket geometry are untouched.

  printed panel  Figure 3 I     (PROVENANCE.csv; NOT inferred from "03_I")
  printed rect   28.6 x 31.9 mm   (panel_rects.csv)
...
PRINTED_MM = (28.6, 31.9)          # published rect, panel_rects.csv
...
def main():
    print("=" * 60)
    print("Panel F: Distance to Stroma Boxplot")
    print("=" * 60)
        ...
        for group in ['CEACAM-high', 'CEACAM-low']:
            ...
            if len(group_df) > 0:
                sample_metrics.append({
                    'sample': sample,
                    'group': group,
                    'distance_to_stroma': group_df['distance_to_stroma'].mean(),
                })
    ...
    create_paired_boxplot(sample_data, 'distance_to_stroma', 'Distance to Stroma',
                         'Distance (a.u.)',
                         os.path.join(BASE_DIR, 'ceacam_spatial_boxplot'), y_max_limit=2300)
