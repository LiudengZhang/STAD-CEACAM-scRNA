#!/usr/bin/env python3
"""
Figure 3 panel K, RESTYLED (Version B) - distance to stroma across the spatial spots
of one Visium section.

NOTE THE PANEL LETTER. This directory is `03_K` and PROVENANCE.csv
confirms it holds printed panel **K** - but the letter was still looked up
there rather than read off the directory name (CLAUDE.md rule 2).

Version A is
`03_Revised_Panels/Main_Figures/03_Figure_3/03_K/create_spatial_dist_stroma.py`
...
same `distance_to_stroma` column, the same `cmap='viridis'`, the same `alpha`, the same
`fraction`/`pad` colour bar and the same equal aspect.

  printed panel  Figure 3 K     (PROVENANCE.csv; NOT inferred from "03_K")
  printed rect   30.7 x 26.4 mm   (panel_rects.csv)
...
PRINTED_MM = (30.7, 26.4)          # published rect, panel_rects.csv
