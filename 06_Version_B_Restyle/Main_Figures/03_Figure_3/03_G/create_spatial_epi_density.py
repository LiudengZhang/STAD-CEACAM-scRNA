#!/usr/bin/env python3
"""
Figure 3 panel G, RESTYLED (Version B) - neighbourhood epithelial density across the spatial spots
of one Visium section.

NOTE THE PANEL LETTER. This directory is `03_G` and PROVENANCE.csv
confirms it holds printed panel **G** - but the letter was still looked up
there rather than read off the directory name (CLAUDE.md rule 2).

Version A is
`03_Revised_Panels/Main_Figures/03_Figure_3/03_G/create_spatial_epi_density.py`
...
same `neighborhood_epi_density` column, the same `cmap='YlOrRd'`, the same `alpha`, the same
`fraction`/`pad` colour bar and the same equal aspect.

  printed panel  Figure 3 G     (PROVENANCE.csv; NOT inferred from "03_G")
  printed rect   29.6 x 27.8 mm   (panel_rects.csv)
...
PRINTED_MM = (29.6, 27.8)          # published rect, panel_rects.csv
...
def main():
    ...
    print("=" * 60)
    print("Panel D: Spatial Epithelial Density")
    print("=" * 60)
