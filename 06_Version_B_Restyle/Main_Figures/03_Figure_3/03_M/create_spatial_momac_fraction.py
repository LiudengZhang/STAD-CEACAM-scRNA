#!/usr/bin/env python3
"""
Figure 3 panel M, RESTYLED (Version B) - the monocyte/macrophage deconvolution fraction across the spatial spots
of one Visium section.

NOTE THE PANEL LETTER. This directory is `03_M` and PROVENANCE.csv
confirms it holds printed panel **M** - but the letter was still looked up
there rather than read off the directory name (CLAUDE.md rule 2).

Version A is
`03_Revised_Panels/Main_Figures/03_Figure_3/03_M/create_spatial_momac_fraction.py`
...
same `Monocytes/Macrophages` column, the same `cmap='YlOrRd'`, the same `alpha`, the same
`fraction`/`pad` colour bar and the same equal aspect.

  printed panel  Figure 3 M     (PROVENANCE.csv; NOT inferred from "03_M")
  printed rect   32.6 x 36.0 mm   (panel_rects.csv)
...
PRINTED_MM = (32.6, 36.0)          # published rect, panel_rects.csv
...
def main():
    ...
    print("=" * 60)
    print("Panel J: Spatial MoMac Fraction")
    print("=" * 60)
    ...
    print(f"  MoMac range: {sample_df['Monocytes/Macrophages'].min():.3f} \u2013 {sample_df['Monocytes/Macrophages'].max():.3f}")
    ...
    out = os.path.join(BASE_DIR, 'spatial_momac_fraction')
