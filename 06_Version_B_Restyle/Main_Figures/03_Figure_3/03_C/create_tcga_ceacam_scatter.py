#!/usr/bin/env python3
"""
Figure 3 panel B, RESTYLED (Version B) - CEACAM5/6 vs CD274 in TCGA-STAD.

NOTE THE PANEL LETTER, AND THE FILENAME. This directory is `03_C` but
PROVENANCE.csv says it holds printed panel **B**. CLAUDE.md rule 2: never infer
a panel letter from a directory name. And `03_D/create_tcga_ceacam_scatter.py`
is a *different file with the same name* drawing printed panel E; the two are
kept in their own directories and must not be merged.

Version A is
`03_Revised_Panels/Main_Figures/03_Figure_3/03_C/create_tcga_ceacam_scatter.py`
...
filter, the Spearman test and the P-value formatting are untouched.
  ...
  printed panel  Figure 3 B     (PROVENANCE.csv; NOT inferred from "03_C")
  printed rect   47.4 x 22.9 mm   (panel_rects.csv)
...
PRINTED_MM = (47.4, 22.9)            # published rect, panel_rects.csv
