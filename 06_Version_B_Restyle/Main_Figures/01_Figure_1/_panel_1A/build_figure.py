...
THIS PANEL IS NOT A MATPLOTLIB PANEL
------------------------------------
It writes raw SVG. `10_Reproduction/compare_panel_content.py` reads matplotlib
artists and therefore cannot gate it - it is the one main-figure drawing the
usual gate does not cover, and saying so is part of the deliverable rather than
a gap to be papered over. `11_Version_B_Restyle/_audit/compare_svg_content.py`
gates it instead: it diffs every text string, every text coordinate and every
piece of non-text geometry between the two SVGs, ignoring font-size and
font-family, which are the restyle.

IT IS ALSO ALREADY 1:1
----------------------
The canvas is 510.24 x 196 pt = 180 x 69.2 mm and the type is set in points on
it, so unlike every other panel in the paper this one already had `set_pt` mean
something. What it did not have is one family and the system's sizes: eleven
distinct sizes from 6.0 to 11 pt, and a font-family *list* that leaves the
choice to whatever renders the SVG - which is one of the mechanisms behind the
thirteen families measured across the shipped figures.

THE THREE CHANGES
-----------------
1. `F` names ONE family, the one this machine will actually draw, taken from
   `panel_style_cns.resolved_family()`. A list here is what let matplotlib draw
   Nimbus Sans and cairo draw Liberation Sans off the identical stack.
2. `T()` maps its `size` through SIZE_MAP onto the cnsplots scale: headings and
   section titles to `body_pt()` (8), everything else to `tick_pt()` (7). The
   sizes are the only numbers that move.
3. The panel's own 11 pt "A" is removed. In Version B the assembler draws every
   panel letter, in one place, at 8 pt bold in the panel-label font; a letter
   drawn here as well would print twice. The rule and arrowhead beside it stay.

The paths are also resolved against this file rather than the process's current
directory, so the script can be run from anywhere. That is robustness, not
content.

  printed panel  Figure 1 A     (PROVENANCE.csv)
  printed rect   224.4 x 86.2 mm on the 254 mm Version A page - i.e. the 180 mm
                 drawing was placed at 1.247x, so its 6.0-11 pt type printed at
                 7.5-13.7 pt (panel_rects.csv)
  Version B box  180.0 x 69.2 mm, placed at 1.000

Pictograms are embedded <symbol>s from an openly licensed scientific icon set:
...
# Run against this file's own directory: Version A read `icons_src/` and wrote
# `figure1A.svg` relative to the process's cwd.
os.chdir(Path(__file__).resolve().parent)

style.apply()
_BODY, _TICK, _LETTER = style.body_pt(), style.tick_pt(), style.letter_pt()

# Version A's eleven sizes, mapped onto the two the system has. 8, 7.2 and 7 pt
# are the panel headings and sub-headings and become the body size; 6.8 and
# below are labels and become the tick/label size. 11 pt was the panel letter
# and no longer appears - see change 3.
SIZE_MAP = {
    8.0: _BODY, 7.2: _BODY, 7.0: _BODY,
    6.8: _TICK, 6.6: _TICK, 6.5: _TICK, 6.4: _TICK, 6.3: _TICK,
    6.1: _TICK, 6.0: _TICK,
    11.0: _LETTER,          # kept in the map so an unmapped size still raises
}

W, H = 510.24, 196.0
F = style.resolved_family()
INK = "#1a1a1a"
...
def load_symbol(sid, fname):
    s = open(os.path.join("icons_src", fname), encoding="utf-8").read()
...
def T(x, y, s, size=6.5, w="normal", anchor="middle", fill=INK, raw=False):
    # The one restyled line: the size asked for is mapped onto the cnsplots
    # scale. An unmapped size raises rather than silently passing through, so
    # a size added later cannot quietly reintroduce a twelfth type size.
    try:
        size = SIZE_MAP[round(float(size), 2)]
    except KeyError:
        raise KeyError(f"font size {size} is not in SIZE_MAP; add it "
                       f"deliberately or use one of {sorted(SIZE_MAP)}")
    b = f' font-weight="{w}"' if w != "normal" else ""
    body = s if raw else esc(s)
    # The family is named on EVERY <text>, not only on the root <svg>.
    # Version A set it once on the root and relied on inheritance, which
    # survives as long as the file is opened on its own - but the assembler
    # embeds the panel's contents into a page, the root element goes, and the
    # inherited family goes with it. Measured on the first Version B build of
    # Figure 1: all 45 of this panel's spans printed in DejaVu Sans while the
    # panel beside it printed in Nimbus Sans. font-family is ignored by
    # `_audit/compare_svg_content.py`, so this changes nothing it checks.
    add(f'<text x="{x:.2f}" y="{y:.2f}" font-family="{F}" font-size="{size}"{b} text-anchor="{anchor}" fill="{fill}">{body}</text>')

IT = '<tspan font-style="italic">n</tspan>'          # AACR: italic lowercase n
def N(v):  return f'{IT} = {v}'                      # "n = 24"
def NP(v): return f'({IT} = {v})' 
...
add(f'<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" '
    f'width="180mm" height="{H/W*180:.2f}mm" viewBox="0 0 {W:.2f} {H:.2f}" font-family="{F}">')
...
# Version A drew its own panel letter here: T(5, 14, "A", 11, "bold", "start").
# In Version B the assembler draws every panel letter, in one place, at 8 pt
# bold in the panel-label font. Drawn here as well it would print twice.
...
T(285, 155, "Stomach, " + N(32), 6.1, raw=True)
...
T(329, 95, "Liver", 6.4);  T(329, 103, N(12), 6.4, raw=True)
T(357, 95, "Ovary", 6.4);  T(357, 103, N(3), 6.4, raw=True)
...
T(329, 156, "LN", 6.4);   T(329, 164, N(12), 6.4, raw=True)
T(357, 156, "PBMC", 6.4); T(357, 164, N(11), 6.4, raw=True)
...
g_close()
add("</svg>")
open("figure1A.svg", "w", encoding="utf-8").write("\n".join(o) + "\n")
...
"""
open("ICON_CREDITS.md", "w", encoding="utf-8").write(credits)
