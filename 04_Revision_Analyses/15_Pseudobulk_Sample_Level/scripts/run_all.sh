...
# and edgeR 4.4.0 are in r_bayesprism, DESeq2 1.50.2 is in deseq2_env, and neither has
...
conda run --no-capture-output -n r_bayesprism Rscript "$HERE/run_limma_voom.R" "$PB" "$DE"
...
for i in $(seq 1 $N); do
  conda run --no-capture-output -n stad_ceacam python \
