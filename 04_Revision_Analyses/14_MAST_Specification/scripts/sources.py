...
changes, both of which the task brief specifies:
  ...
  1. The eleven shared-pipeline types are read from 06_Clean_Data/01_H5AD/
     (the rebuilt, singly-normalised objects) rather than Round_5.
...
Mast and plasma cells have no file in 01_H5AD and keep the Round_4 objects the
...
# Round_5/01_Raw_Inputs/01_H5AD/Pericyte.h5ad `.raw.X` (max abs difference
# 0.0, same 5,205 cells and 57,058 genes in the same order), which is the file
# the 2026-08-31 sound run read.
#
# The rebuilt neutrophil object and the two Round_4 objects do carry `.raw`,
