...
# recompute_summary_Neutrophils_<phase>.csv - the same file names the
# 2026-08-31 sound run produced for the other twelve types.
#
...
ENV_LIB=/path/to/home/miniforge3/envs/Liudeng_Python_310/lib
for phase in pre post; do
  echo "=== $phase ==="
  LD_LIBRARY_PATH="$ENV_LIB" conda run --no-capture-output \
      -n Liudeng_Python_310 python "$HERE/recompute_deg_neutrophils.py" \
